...
model_names = sorted(name for name in model_Gate.__dict__
                     if name.islower() and not name.startswith('__')
                     and callable(model_Gate.__dict__[name])
                     )
print(model_names)
def parse_args():
    ...
    parser.add_argument('arch', metavar='ARCH',
                        default='cifar10_feedforward_34',
                        choices=model_names,
...
def run_training(args):
    # create model
    model = model_Gate.__dict__[args.arch]().to(device)
    best_prec1 = 0
